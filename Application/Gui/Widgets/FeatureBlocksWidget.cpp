...
void FeatureBlocksWidget::setupButtons()
{
    ...
    for(const QString& qstrFeatureBlockId : qstrFeatureBlockIds)
    {
        ...
        ptrAction->setText(ptrFeatureBlock->featureBlockInfo()->pluginDisplayName());//featureBlockDisplayName());
        ptrAction->setIcon(ptrFeatureBlock->featureBlockInfo()->pluginIcon());//featureBlockIcon());
...
void FeatureBlocksWidget::retranslateFeatureBlockWidgets()
{
    ...
    while(iterator != m_qhashFeatureBlockWidgets.constEnd())
    {
        ...
        if(ptrWidget)
        {
            ...
            if(ptrFeatureBlock)
            {
                ptrWidget->setTitle(ptrFeatureBlock->featureBlockInfo()->pluginDisplayName());//featureBlockDisplayName());
            }
        }
